 ...
 * @version   $Id: Db.php 2014-04-07 11:12:45 tak@ $
 */
...
class P5_Db
{
    ...
    public function __construct($driver, $host, $source, $user, $password, $port=3306, $enc='')
    {
        ...
        if($this->_driver != 'sqlite' && $this->_driver != 'sqlite2') {
            ...
            if(!file_exists($host)) mkdir($host, 0777, true);
            $this->_dsn = "$driver:$host/$source";
            if(!empty($port)) $this->_dsn .= ".$port";
        }
    }
    ...
    public function updateOrInsert($table, array $data, $unique, $raws = array())
    {
        ...
        foreach($data as $unit) {
            ...
            foreach ($unique as $key) {
                $where[] = $unit[$key];
                $arr[] = "{$key} = ?";
                if(in_array($key, $keys)) unset($update[$key]);
            }
    ...
    public function replace($table, array $data, $unique, $raws = array(), $fields = null)
    {
        ...
        foreach($data as $unit) {
            ...
            if(empty($cols)) $cols = implode(',', $keys);
            ...
            if(false === $ret = $this->exec($sql)) return false;
            $ecount += $ret;
        }
        $this->_ecount = $ecount;
        return $this->_ecount;
    ...
    public function select($columns, $table, $statement = '', $options = array())
    {
        ...
        if($this->query($sql)) return $this->fetchAll();
        return false;
    }
    ...
    public function get($column, $table, $statement = '', $options = array())
    {
        ...
        if($this->query($sql)) return $this->fetchColumn();
        return false;
    }
    ...
    public function begin($identifire = '')
    {
        if($this->_dbHandler->inTransaction() || $this->_dbHandler->beginTransaction()) {
            if(!empty($identifire)) $this->query("SAVEPOINT $identifire");
    ...
    {
        if(is_null($table)) return $this->_dbHandler->lastInsertId($col);
     ...
     * @return boolean
     */
    static public function is_number($type)
    {
    ...
    public function errorInfo()
    {
        if(is_object($this->_dbResult)) {
            ...
            if(!is_null($info[2])) return $info;
        }
        if(is_null($this->_dbHandler)) return null;
     ...
     * @return string
     */
    static public function countSQL($sql)
    {
        $arr = array();
