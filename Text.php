...
class P5_Text
{
     ...
     * @return string
     */
    static public function wrap($text, $col, $delimiter, $enc = 'utf-8')
    {
		$tmp = preg_split("/(\r\n|\r|\n)/", $text);
		$line = '';
		foreach ($tmp as $str) {
	 ...
	 * @return string
	 */
	static public function convert($str, $encodingTo = 'utf-8', $encodingFrom = null) 
	{
        if (empty($encodingTo)) {
     ...
     * @return string
     */
    static public function detect_encoding($str)
    {
        // Unicode
        $encoding = self::detect_utf_encoding($str);
        if (!empty($encoding)) return $encoding;
	 ...
	 * @return string
	 */
	static public function implode() 
	{
        $asset = func_get_args();
     ...
     * @return string
     */
    static public function detect_utf_encoding($str)
    {
        $f2 = substr($str, 0, 2);
        $f3 = substr($str, 0, 3);
        $f4 = substr($str, 0, 3);

        if ($f3 == UTF8_BOM) return 'UTF-8BOM';
        elseif ($f4 == UTF32_BIG_ENDIAN_BOM) return 'UTF-32BE';
        elseif ($f4 == UTF32_LITTLE_ENDIAN_BOM) return 'UTF-32LE';
        elseif ($f2 == UTF16_BIG_ENDIAN_BOM) return 'UTF-16BE';
        elseif ($f2 == UTF16_LITTLE_ENDIAN_BOM) return 'UTF-16LE';
    }
     ...
     * @return string
     */
    static public function removeBOM($str, $bom = UTF8_BOM)
    {
     ...
     * @return  boolean
     */
    static public function convertBoolean($str) 
    {
     ...
     * @return mixed
     */
    static public function checkEncodings($enc)
    {
     ...
     * @return bool
     */
    static public function is_blank($var)
    {
