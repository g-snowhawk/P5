...
class P5_Html_Form
{
    ...
    public function getAction(P5_Html_Source $html, $id)
    {
        ...
        if (is_object($form)) return $form->getAttribute('action');
        return;
    }
    ...
    public function setAction(P5_Html_Source $html, $id, $action, $force = false)
    {
        ...
        if (is_object($form)) {
            ...
            if (!empty($def) && $force == false) return;
            if ($def != $action) $form->setAttribute('action', $action);
        }
    }
    ...
    public function setEnctype(P5_Html_Source $html, $id, $enctype = 'application/x-www-form-urlencoded')
    {
        ...
        if (is_object($form)) {
            ...
            if ($def != $enctype) $form->setAttribute('enctype', $enctype);
        }
    }
    ...
    public function setMethod(P5_Html_Source $html, $id, $method)
    {
        ...
        if (is_object($form)) {
            ...
            if (empty($def)) {
                if ($def != $method) $form->setAttribute('method', $method);
            }
        }
    }
    ...
    public function getValue($name, $force = null)
    {
        ...
        if (!empty($force)) $data = (strtolower($force) == 'post') ? 'POST' : 'GET';
     ...
     * @param  boolean  $optonly
     * @return string
     */
    static public function prefSelector($name, $selected, $optonly = 0, $label = '')
    {
        $src = '';
        if (! $optonly) $src .= '<select name="' . $name . '" id="' . $name . '">' . "\n";
        ...
        foreach ($prefs as $pref) {
            ...
            if ($pref == $selected) $src .= ' selected="selected"';
            $src .= '>' . $pref . '</option>' . "\n";
        }
        if (! $optonly) $src .= '</select>' . "\n";
    ...
    {
        if (strtolower($method) != 'post') return array_key_exists($key, $_GET);
